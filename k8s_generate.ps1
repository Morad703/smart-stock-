...
type: Opaque
data:
  # Base64 encodings pour Mots de passe
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: config-server
        image: mouradmounib/config-server:latest
        ports:
        - containerPort: 8888
        readinessProbe:
          httpGet:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: eureka-server
        image: mouradmounib/eureka-server:latest
        ports:
        - containerPort: 8761
        readinessProbe:
          httpGet:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: api-gateway
        image: mouradmounib/api-gateway:latest
        env:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: ms-${name}
        image: mouradmounib/ms-${name}:latest
        env:
        - name: MYSQL_HOST
          valueFrom:
            configMapKeyRef:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: frontend
        image: mouradmounib/frontend:latest
        ports:
        - containerPort: 80
---
apiVersion: v1
